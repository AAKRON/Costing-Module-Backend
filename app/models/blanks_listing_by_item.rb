class BlanksListingByItem < ApplicationRecord
  ...
  def self.listing_xlsx(blanks)
    ...
    wb.add_worksheet(name: "Blanks Item") do |sheet|
      sheet.add_row ["ID", "ITEM", "BLANK NUMBER", "MULT","DIV"]
      blanks.each do |result|
        ...
        sheet.add_row [result.id, item_description, result.blank_number, result.mult, result.div]
      end
    end
