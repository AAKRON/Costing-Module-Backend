...
class SetupController < ApplicationController
  ...
  def schema_load
    ...
    render json: { status: 'success', message: 'Database schema loaded successfully from schema.rb', timestamp: Time.current }
  rescue => e
  ...
  def migrate
    ...
    render json: { status: 'success', message: 'Database migrations completed successfully', timestamp: Time.current }
  rescue => e
    render json: { status: 'error', message: e.message, timestamp: Time.current }, status: 500
  end
  ...
  def seed
    ...
    render json: { status: 'success', message: 'Database seeding completed successfully', timestamp: Time.current }
  rescue => e
    render json: { status: 'error', message: e.message, timestamp: Time.current }, status: 500
  end
  ...
  def create_test_user
    ...
    if user.save
      ...
      render json: { status: 'error', message: 'Failed to create user', errors: user.errors.full_messages, timestamp: Time.current }, status: 422
    end
  rescue => e
    render json: { status: 'error', message: e.message, timestamp: Time.current }, status: 500
  end
  ...
  def reset_user_password
    ...
    if user.save
      ...
      render json: { status: 'error', message: 'Failed to save user', errors: user.errors.full_messages, timestamp: Time.current }, status: 422
    end
  rescue => e
    render json: { status: 'error', message: e.message, timestamp: Time.current }, status: 500
  end
