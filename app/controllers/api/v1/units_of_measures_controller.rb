module Api
  module V1
    class UnitsOfMeasuresController < BaseController
      ...
      after_action (only: [:index]) { set_pagination_header(UnitsOfMeasure.count) }
      after_action (only: [:units_of_measure_list_only]) { set_pagination_header(UnitsOfMeasure.count) }
