class FixViewBusinessKeyJoins < ActiveRecord::Migration[7.2]
  def up
    ...
    # blank_cost_views fixes:
    #   RIGHT JOIN blanks b ON b.blank_number = bc.blank_id  (was b.id -- blank_jobs.blank_id stores blank_number business key)
    #   LEFT JOIN blank_average_costs bac ON bac.blank_id = b.id  (was bc.blank_id -- bac.blank_id is a FK to blanks.id)
    execute <<~SQL
      ...
      RIGHT JOIN blanks b ON b.blank_number = bc.blank_id
      LEFT JOIN blank_average_costs bac ON bac.blank_id = b.id
      LEFT JOIN blank_types bt ON b.blank_type_id = bt.type_number;
    SQL

    # item_with_blank_per_cost_views fix:
    #   LEFT JOIN blank_cost_views bcv ON bcv.blank_number = bliwc.blank_number  (was bcv.id)
    execute <<~SQL
    ...
    # item_cost_views fix:
    #   ibc ON ibc.item_number = i.item_number  (was i.id)
    execute <<~SQL
