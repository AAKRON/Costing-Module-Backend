class UpdateItemCostLocationFunction < ActiveRecord::Migration[6.1]
    def up
        connection.execute(%q(
            ...
            BEGIN
                ...
                CAST((COALESCE(secondary_box.cost_per_box, 0) / CASE WHEN  i.number_of_pcs_per_secondary_box = 0 THEN 1 ELSE i.number_of_pcs_per_secondary_box END) AS DECIMAL(10,5)) AS secondary_box_cost,
                CAST(COALESCE(COALESCE(ibc.item_blank_cost_for_price, 0)
                    +
                    ...
                    +
                    CAST((COALESCE(secondary_box.cost_per_box, 0) / CASE WHEN  i.number_of_pcs_per_secondary_box = 0 THEN 1 ELSE i.number_of_pcs_per_secondary_box END) AS DECIMAL(10,5))
                    +
                    COALESCE(ijcws.screen_cost, 0)
                    +
                    COALESCE(inks.ink_cost, i.ink_cost)
                , 0) AS DECIMAL(10,5)) AS total_price_cost,
