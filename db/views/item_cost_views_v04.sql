select
  ...
  CAST((COALESCE(ibc.item_blank_cost_for_price, 0)
   ...
   +
   i.ink_cost
  ) AS DECIMAL(10,4)) AS total_price_cost,
  CAST((COALESCE(ibc.item_blank_cost_for_inventory, 0)
   ...
   +
   i.ink_cost
  ) AS DECIMAL(10,4)) AS total_inventory_cost
FROM items i
LEFT JOIN boxes b
ON i.box_id = b.id
LEFT JOIN (
...
LEFT JOIN (
      SELECT
        ...
        CAST(
          SUM(
            (CAST((jl.wages_per_hour * CAST(ij.hour_per_piece AS DECIMAL(10,4))) AS DECIMAL(10,4)))
            +
            (CAST((jl.wages_per_hour * CAST(ij.hour_per_piece AS DECIMAL(10,4))) AS DECIMAL(10,4)) * CAST(acpo.value AS numeric))
          )
        ...
        CAST(
          SUM(
            (CAST((jl.wages_per_hour * CAST(ij.hour_per_piece AS DECIMAL(10,4))) AS DECIMAL(10,4)))
            +
            (CAST((jl.wages_per_hour * CAST(ij.hour_per_piece AS DECIMAL(10,4))) AS DECIMAL(10,4)) * CAST(acio.value AS numeric) )
          )
